...
export async function initBrowser(): Promise<boolean> {
    ...
    const config = pluginState.config.browser;
    pluginState.log('info', `浏览器配置: executablePath=${config.executablePath || 'auto'}, browserWSEndpoint=${config.browserWSEndpoint || 'none'}, headless=${config.headless}`);
